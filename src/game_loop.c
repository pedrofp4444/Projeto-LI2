...
int game_loop_init_ncurses(void) {
	if (initscr() == NULL)
		return 1;
	if (start_color() == ERR)
		return 1; /* Enable color support */
	if (cbreak() == ERR)
		return 1; /* Disable line buffering and control characters */
	if (noecho() == ERR)
		return 1; /* Don't show input on the terminal */
	if (nodelay(stdscr, 1) == ERR)
		return 1; /* Make getch return ERR if no input is available */
	if (nonl() == ERR)
		return 1; /* Transform new lines intro carriage returns */
	if (keypad(stdscr, 1) == ERR)
		return 1; /* Let ncurses parse escape sequences */
	if (curs_set(0) == ERR)
		return 1; /* Hide the cursor */
...
game_loop_callback_return_value game_loop_window_size(void *state, int *width, int *height,
	...
	if (!(nwidth == *width && nheight == *height)) { 
		...
		if (onresize) {
			...
			if (ret != GAME_LOOP_CALLBACK_RETURN_SUCCESS)
				return ret;
		}
 ...
 *          ::GAME_LOOP_CALLBACK_RETURN_SUCCESS is returned.
 */
game_loop_callback_return_value game_loop_handle_input(void *state, game_loop_input_callback oninput) {

	if (oninput) {
		/* Skip reading input if no input callback is defined */
		int c = getch();
		while (c != ERR) {
...
int game_loop_keep_fps(struct timespec frame_start, double frame_time) {
	...
	if (clock_gettime(CLOCK_MONOTONIC, &frame_end))
		return 1;
...
int game_loop_run(void *state, game_loop_callbacks callbacks, unsigned int fps) {
	...
	if (clock_gettime(CLOCK_MONOTONIC, &last_frame_instant))
		return 1;
	...
	if (fps)
		frame_time = 1.0 / fps;
	if (fps == 1)
		frame_time = 0.9999; /* Edge case when converting to timespec */

	while (1) {
		/* Calculate frame time (since the beginning of the last frame) */
		struct timespec frame_instant;
		if (clock_gettime(CLOCK_MONOTONIC, &frame_instant))
			return 1;
		...
		if (callbacks.onupdate)
			ret = callbacks.onupdate(state, delta);
		game_loop_return(ret);

		if (callbacks.onrender)
			ret = callbacks.onrender(state, width, height);
		game_loop_return(ret);

		if (game_loop_keep_fps(frame_instant, frame_time))
			return 1;
	}
